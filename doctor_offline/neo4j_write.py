...
def _load_data(path):
    ...
    print(f"Found {len(disease_csv_list)} files in the directory.")
    ...
    for disease_csv in disease_csv_list:
        ...
        print(f"File {disease_csv} contains {len(symptom)} symptoms.")
...
def write(path):
    ...
    with driver.session() as session:
        ...
        # 创建Disease节点的索引
        cypher = "CREATE INDEX IF NOT EXISTS FOR (d:Disease) ON (d.name)"
        session.run(cypher)
        # 创建Symptom节点的索引
        cypher = "CREATE INDEX IF NOT EXISTS FOR (s:Symptom) ON (s.name)"
        session.run(cypher)


if __name__ == '__main__':
    path = r"C:\Users\qq178\work\AI_doctor\doctor_offline\structured\reviewed"
    write(path)
